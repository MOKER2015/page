...
//colNUM://分成多少列来显示
//openLink是否在地区选择器上开启站点链接true-开启；false-关闭
function Selector(InstanceName, sType, maxNum, colNUM, openLink) {
	// 对象名称
...
function setType(sType) {
	if (sType == 0) {// 省市选择
		...
		this.title1 = "返回行业";
	} else if (sType == 10) {// 语言技能
		// 选择器的标题
		this.selectorName = "语言";
		this.typeAry = catelogsLang;
		this.typeAryLength = catelogsLangCount;
		this.title1 = "返回";
	}
}
...
/**
 * pos:当前点击的类别的数组位置 objBox:但前点击的类别的复选框对象
 */
...
function showResult() {
	...
	if (chooseResult != "") {
		...
		itemHtml = "<table id=\""
				+ this.resultTblId
				+ "\" width=\"100%\" class=\"TableFSTd\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
		var sitem = "";
		for ( var i = 0; i < aryChooseResult.length; i++) {
			...
			for ( var j = 0; j < this.colNUM; j++) {
				...
				if (h != -1) {
					...
					sitem = "<td height=\""
							+ this.rowHeight
							+ "\"><input type=\"checkbox\" style=\"width: 13px\" name=\""
							+ this.instanceName + this.typeAry[h][3]
							+ "result\" id=\"" + this.instanceName
							+ this.typeAry[h][3]
							+ "result\" checked onclick=\"" + this.instanceName
							+ ".clickResult(" + h + ")\" value=\""
							+ this.typeAry[h][5]
							+ "\"><a href=\"####\" onclick=\""
							+ this.instanceName + ".clickResult(" + h + ")\">"
							+ this.typeAry[h][0] + "</a></td>";
					itemHtml += sitem;
					isAdd = 1;
	...
	if (objdiv != null) {
		...
		if (itemHtmlBut == "") {
			itemHtmlBut = "请选择";
			document.getElementById(this.dirName+"0").value="";
		}else{
			document.getElementById(this.dirName+"0").value=itemHtmlBut;
		}
 ...
 * 
 * @param pos
 * 选择的类型的数组下标 @ param aryChooseResult 结果数组 @ return 过滤后的结果数组
 */
function filterSon(pos, aryChooseResult) {
	if (this.isFather(pos) == false)
...
function isFather(pos) {
	if (this.typeAry[pos][4] == 3)
		return false;
	var nextPos = pos + 1;
	if (nextPos >= this.typeAryLength)
		return false;
	if (this.typeAry[pos][4] == (this.typeAry[nextPos][4] - 1))
		return true;
	return false;
}
...
function updateSonCheck(pos, isForbid) {
	...
	for ( var i = pos + 1; i < this.typeAryLength; i++) {
		...
		if (this.typeAry[i][2] == parentId) {
			if (this.typeAry[i][4] == sonLevel
					|| this.typeAry[i][4] == sonLevel + 1) {
...
function setTitleHtml(pos, isChecked) {
	...
	var itemHtml = "<table width=\"100%\" border=\"0\" class=\"TableFSTd\" cellspacing=\"0\" cellpadding=\"0\">";
	if (this.typeAry[pos][4] >= this.selectLevel) {// 可以选择
		if(this.sType==10){
			itemHtml += "<tr><td height=\"" + this.rowHeight + "\">" + spaceStr
			+ "<input  disabled "
			+ " type=\"checkbox\" style=\"width: 13px\" name=\""
			+ this.instanceName + pos + "\" id=\"" + this.instanceName
			+ pos + "\" onclick=\"" + this.instanceName + ".clickTitleBox("
			+ pos + ",this);\" value=\"" + this.typeAry[pos][5]
			+ "\"><a style=\"cursor:pointer\" onclick=\"javascript:"
			+ this.instanceName + ".clickTitleName(" + pos + ");\">"
			+ this.typeAry[pos][0] + "</a><span class=\"hong\">"
			+ this.parentTitle + "</span></td></tr></table>";
		}else{
			itemHtml += "<tr><td height=\"" + this.rowHeight + "\">" + spaceStr
				+ "<input  " + strChecked
				+ " type=\"checkbox\" style=\"width: 13px\" name=\""
		...
		itemHtml += "<tr><td height=\"" + this.rowHeight
				+ "\"><span style='cursor:pointer' onclick='"
				+ this.instanceName + ".clickContentName(" + pos + ");'>"
				+ spaceStr + this.typeAry[pos][0] + "</span></td></tr></table>";
	}
...
function assembleAll() {
	...
	var objTitle2 = document.getElementById(this.title2Id);
	if (objTitle2.innerHTML == "" || this.title2 == "")
		objTitle2.innerHTML = this.title2;

	var objSpan2 = document.getElementById(this.span2Id);
	if (objSpan2.innerHTML == "" || this.content2 == "")
		objSpan2.innerHTML = this.content2;

	var objTitle3 = document.getElementById(this.title3Id);
	if (objTitle3.innerHTML == "" || this.title3 == "")
		objTitle3.innerHTML = this.title3;

	var objSpan3 = document.getElementById(this.span3Id);
	if (objSpan3.innerHTML == "" || this.content3 == "")
		objSpan3.innerHTML = this.content3;
...
function setDefaultShow(firstResult, defaultShow) {
	 ...
	 * if(this.sType==0){ defaultShow="18";//显示山东省下的市区 }else
	 * if(this.sType==3){//地铁 defaultShow="398";//北京 }else if(this.sType==6){
	 * defaultShow="25";//烟台 } }
	 */
	return defaultShow;
}
...
function setContent(pos, isChecked) {
	...
	var level = 1;
	if (pos != -1)
		level = this.typeAry[pos][4] + 1;
	var spaceStr = this.getSpace(level);
	var itemHtml = "<table id=\""
			+ this.instanceName
			+ "table"
			+ level
			+ "\" class=\"TableFSTd\" width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
	var sitem = "";// 每个类型的html
	if(this.sType==10){
		for ( var i = pos + 1; i < this.typeAryLength; i++) {
			...
			for ( var j = 0; j < this.colNUM; j++) {
				if (pos == -1 || this.typeAry[i][2] == this.typeAry[pos][2]) {// 判断是否列第一级或数组元素和被点击的元素是否是同一父级
					if (this.typeAry[i][4] == level) {// 是否是当前要列的级
						if (this.typeAry[i][4] >= this.selectLevel) {// 可以选择的级别
							...
							sitem = "<td height=\""
									...
									+ "&nbsp;<input"
									+ strDisabled
									+ " type=\"checkbox\" style=\"width: 13px\" id=\""
									+ this.instanceName
									+ i
									+ "\" name=\""
									+ this.instanceName
									...
									+ ".clickContentName("
									+ i
									+ ",0);\"><a style=\"cursor:pointer\" onclick=\"javascript:"
									+ this.instanceName + ".clickContentName(" + i
									+ ",1);\">" + this.typeAry[i][0] + "</a></td>";
						} else {
							// 如果选框的值存在已选择结果中则选框变为选中
							// isChecked=this.returnCheckStatus(chooseResult,i);
							sitem = "<td height=\""
									...
									+ spaceStr
									+ "&nbsp;<a style=\"cursor:pointer\" onclick=\"javascript:"
									+ this.instanceName + ".showSon(" + i
									+ ",null);\">" + this.typeAry[i][0]
									+ "</a></td>";
						}
						itemHtml += sitem;
						isAdd = 1;
					...
					i = this.typeAryLength;// alert("i===" + i+"********" +
											// this.typeAryLength);
					break;
				}
...
function selectorInit(oResultFieldId, butId, pos, offset) {
	...
	var divHtml = "";
	if (offset == null) {
		divHtml = "<DIV id=\"" + this.divId + "\"  class=\"" + divClass
				+ "\" style='display:none; Z-INDEX:999'>";
	} else {
		divHtml = "<DIV id=\"" + this.divId + "\"  class=\"" + divClass
				+ "\" style='display:none; Z-INDEX:999; margin:24px 0 0 "
				+ offset + "px'>";
	}
	divHtml += "<table width=\"" + this.divWidth
			+ "px\" border=\"0\"  cellpadding=\"0\" cellspacing=\"0\">";
	divHtml += "<tr>";
	/*
	 * divHtml+="<td width=\"9\" height=\"9\">"; divHtml+="<img
	 * src=\"/themes/default/selector/images/typeselector/div_01.gif\"></td>";
	 * divHtml+="<td background=\"/themes/default/selector/images/typeselector/div_02.gif\">";
	 * divHtml+="</td>"; divHtml+="<td>"; divHtml+="<img
	 * src=\"/themes/default/selector/images/typeselector/div_03.gif\"></td>";
	 * divHtml+="</tr>"; divHtml+="<tr>"; divHtml+="<td background=\"/themes/default/selector/images/typeselector/div_04.gif\">";
	 * divHtml+="</td>";
	 */divHtml += "<td valign=\"top\" bgcolor=\"#FFFFFF\"><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" id=\"divtt\">";
	divHtml += "<tr>";
	divHtml += "<td class=\"tt\">" + this.selectorName
			+ "选择器<span class=\"hong\">";
	if (this.maxNum > 0)
		divHtml += "(最多可选" + this.maxNum + "项)</span></td>";
	divHtml += "<td align=\"right\"><span style='cursor:pointer' onclick=\"javascript:divClose('"
			+ this.divId
			+ "');\">[确定]</span> <a href=\"help.html\" target=\"_blank\" style='display:none'>[帮助]</a></td>";
	divHtml += "</tr>";
	divHtml += "</table>";
	divHtml += "<table width=\"100%\" border=\"0\" cellspacing=\"2\" cellpadding=\"0\" align=\"center\">";
	divHtml += "<tr>";
	divHtml += "<td class=\"dl\"><span style='cursor:pointer' onclick=\"javascript:"
			+ this.instanceName
			+ ".setContent1();"
			+ this.instanceName
			+ ".clearContent2();"
	...
	divHtml += "<tr><td class=\"dl\">您已经选择的" + this.selectorName
			+ "是:<span class=\"hong\">(点击可以取消选择)</span></td></tr>";
	divHtml += "<tr><td>";
	divHtml += "<span id=\"" + this.spanResultId + "\"></span>";
	divHtml += "</td></tr></table>";
	divHtml += "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" id=\"divtt\">";
	divHtml += "<tr>";
	divHtml += "<td align=\"right\" height=\""
			+ this.rowHeight
			+ "\" ><span style='cursor:pointer' onclick=\"javascript:divClose('"
			+ this.divId
			+ "');\">[确定]</span> <a href=\"help.html\" target=\"_blank\" style='display:none'>[帮助]</a></td>";
	/*
	 * divHtml+="</tr>"; divHtml+="</table></td>"; divHtml+="<td background=\"/themes/default/selector/images/typeselector/div_06.gif\">";
	 * divHtml+="</td></tr>"; divHtml+="<tr><td>"; divHtml+="<img
	 * src=\"/themes/default/selector/images/typeselector/div_07.gif\"></td>";
	 * divHtml+="<td background=\"/themes/default/selector/images/typeselector/div_08.gif\"></td>";
	 * divHtml+="<td width=\"1\" height=\"1\">"; divHtml+="<img
	 * src=\"/themes/default/selector/images/typeselector/div_09.gif\"></td>";
	 */
	divHtml += "</tr></table></div>";
	if (this.openLink)
		divHtml += this.linkValue;
	// document.getElementById("sthowhtml").value=divHtml;
...
function setFrmHeight() {
	/**
	 * var totalRow=0; var totalheight=93; var
	 * objTable1=document.getElementById(this.table1Id); if(objTable1!=null){
	 * totalRow+=objTable1.rows.length; }
	 * 
	 * var objTable2=document.getElementById(this.table2Id);
	 * if(objTable2!=null){ totalRow+=objTable2.rows.length; } var
	 * objTable3=document.getElementById(this.table3Id); if(objTable3!=null){
	 * totalRow+=objTable3.rows.length; }
	 * oo=document.getElementById(this.resultTblId); if(oo!=null){
	 * totalRow+=oo.rows.length; } if(this.title2!="") totalRow++;
	 * if(this.title3!="") totalRow++;
	 * 
	 * var ofrm=document.getElementById(this.frmId);
	 * totalheight+=totalRow*this.rowHeight; ofrm.style.height=totalheight+"px";
	 */
}

// 得到地区选择器上的站点链接数组
function getLinkArry() {
	var k = 0, linkValue = "";
	for ( var k = 0; k < this.linkAryLength; k++) {
		linkValue += "<a href=\"" + this.linkAry[k][0]
				+ "\" target=\"_blank\">[" + this.linkAry[k][1] + "]</a>&nbsp;";
	}
